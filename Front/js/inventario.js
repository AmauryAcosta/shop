...
const BASE_URL = "http://localhost:8888/shop/Back/inventario.php"; // Asegúrate de que esta URL sea correcta
...
async function obtenerProductos() {
  try {
    ...
    productos.forEach((producto) => {
      ...
      option.value = producto.id_producto; // Asegúrate de que este campo existe en tu objeto de producto
      option.textContent = producto.nombre_producto; // Asegúrate de que este campo existe en tu objeto de producto
  ...
  try {
    ...
    inventarios.forEach((inventario) => {
      ...
      row.innerHTML = `
                ...
                <td>
                    <button class="btn btn-warning btn-sm" onclick="editarInventario(${inventario.id_inventario})">
...
document.addEventListener("DOMContentLoaded", () => {
  obtenerProductos(); // Cargar productos al iniciar
  obtenerInventarios(); // Cargar inventarios al iniciar
});
 */
...
async function editarInventario(id_inventario, cantidadActual) {
  ...
  try {
    const response = await fetch(BASE_URL, {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        id_inventario, // Enviar solo el id_inventario y la nueva cantidad
    ...
    const result = await response.json();
    alert(result.message); // Mensaje de éxito o error de la API
    obtenerInventarios(); // Actualizar la lista de inventarios
  } catch (error) {
    console.error("Error:", error);
